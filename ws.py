...
class WebsocketClient:
	...
	def _auth(self, password):
		...
		if result['authRequired']:
			secret = base64(hashlib.sha256((password + result['salt']).encode('utf-8')).digest())
			auth = base64(hashlib.sha256(secret + result['challenge'].encode('utf-8')).digest()).decode('utf-8')
